...
// API Base URL - use same origin as frontend (no need for separate port)
// Browser code cannot access process.env, only server-side can
const API_BASE_URL = '';
...
export async function fetchUserAgentComparison(
  ...
  limit: number = 10
): Promise<import('../types/analytics').UserAgentComparison> {
  try {
    ...
    const data: ApiResponse<import('../types/analytics').UserAgentComparison> = await response.json();
